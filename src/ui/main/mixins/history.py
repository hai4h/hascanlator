...
class HistoryMixin:
    ...
    def commit_history(self, desc, aggregate=False):
        ...
        if aggregate and curr_idx >= 0:
            ...
            if (
                last_step["desc"] == desc
                and last_step.get("sel_ids") == current_sel_ids
            ):
    ...
    def load_history_step(self, index):
        path = self.workspace.current_image_path
        if not path or path not in self.workspace.history:
            return
        ...
        if step["image"] is None:
            ...
            if img_to_use is not None:
                ...
                self.workspace.edited_images[path] = self.workspace.original_images[
                    path
                ].copy()
        else:
